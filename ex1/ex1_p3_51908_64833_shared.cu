...
__global__ void CUDA_convolution_kernel(pixel_t *A, pixel_t *B, float *kirnel, int nx, int ny, int kn)
{
	...
	if( (threadIdx.x > 0) && (threadIdx.y == 0) && (threadIdx.x < blockDim.x - 1) && (blockIdx.y != 0)){
		a_shared[sid - sdimx] = A[id - (nx)];
	}
	else if( (threadIdx.x > 0) && (threadIdx.y == blockDim.y - 1) && (threadIdx.x < blockDim.x - 1) && (blockIdx.y != gridDim.y - 1)){
		a_shared[ sid + sdimx ] = A[id + (nx)];
	}
	else if( (threadIdx.x == blockDim.x - 1) && (threadIdx.y > 0) && (threadIdx.y < blockDim.y - 1) && (blockIdx.x != gridDim.x - 1)){
		a_shared[ sid + 1 ] = A[id + 1];
	}
	else if( (threadIdx.x == 0) && (threadIdx.y > 0) && (threadIdx.y < blockDim.y - 1) && (blockIdx.x != 0)){
		a_shared[ sid - 1 ] = A[id - 1];
	}
	else if( (threadIdx.x == 0) && (threadIdx.y == 0)){
		a_shared[sid - sdimx - 1] = A[id - (nx) - 1];
		a_shared[sid - sdimx] = A[id - (nx)];
		a_shared[sid - 1] = A[id -1];
	}
	...
	for (int j = -khalf; j <= khalf; j++){
    	for (int i = -khalf; i <= khalf; i++) {
    		pixel += a_shared[ (threadIdx.y + 1 + j) * 18  + (threadIdx.x + 1 + i) ] * kirnel[c];
    		c++;
        }
    }
