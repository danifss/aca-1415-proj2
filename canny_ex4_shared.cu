...
__global__ void CUDA_hysteresis_edges_kernel(const pixel_t *nms, pixel_t *reference, const int nx, const int ny, const int tmin, bool *pchanged)
{
	...
	if ((xx>0 && xx<nx-1) && (yy>0 && yy<ny-1))
	/*  if (!firstPixelColumnofImage  &&  !lastPixelRowofImage) && (!firstPixelRowofImage && !lastPixelRowofImage)  */
	//if(((threadIdx.x>0 && blockIdx.x!=0) && (threadIdx.x<blockDim.x-1 && xx!=nx-1)) && ((threadIdx.y>0 && blockIdx.y!=0) && (threadIdx.y<blockDim.y-1 && yy!=ny-1)))
	{
