...
{	/*
	...
	__shared__ pixel_t reference_shared[BLOCKSIZE];

	reference_shared[ids] = reference[id];
...

	{
		...
		nbs[0] = ids - blockDim.x;     // nn
		nbs[1] = ids + blockDim.x;     // ss
		nbs[2] = ids + 1;      // ww
		nbs[3] = ids - 1;      // ee
		nbs[4] = nbs[0] + 1; // nw
		nbs[5] = nbs[0] - 1; // ne
		...
		if (nms[id] >= tmin && reference_shared[ids] == 0) {
			for(int k = 0; k < 8; k++){
				if (reference_shared[nbs[k]] != 0) {
					reference_shared[ids] = MAX_BRIGHTNESS;
					__syncthreads();
					*pchanged = true;
			...
			reference[id] = reference_shared[ids];
		}
		
	}*/
	
	int sdimx = blockDim.x + 2;
	int sdimy = blockDim.y +2;
	int m = threadIdx.x + blockIdx.x * blockDim.x;
	int n = threadIdx.y + blockIdx.y * blockDim.y;
	int id = m + n * nx;

	int sid = (threadIdx.x + 1) + ((threadIdx.y + 1) * sdimx); 
	...
	__shared__ pixel_t a_shared[CONVSIZE];
	//cudaMemset( a_shared, 0, BLOCKSIZE*sizeof(pixel_t));
	
	
	a_shared[sid] = reference[id];
	...
	/*  se sid for da primeira linha mas não o primeiro nem o último  */
	if( (threadIdx.x > 0) && (threadIdx.y == 0) && (threadIdx.x < blockDim.x - 1) ){
		a_shared[sid - sdimx] = reference[id - (nx)];
	}
	else if( (threadIdx.x > 0) && (threadIdx.y == blockDim.y - 1) && (threadIdx.x < blockDim.x - 1)){
		a_shared[ sid + sdimx ] = reference[id + (nx)];
	}
	else if( (threadIdx.x == blockDim.x - 1) && (threadIdx.y > 0) && (threadIdx.y < blockDim.y - 1)){
		a_shared[ sid + 1 ] = reference[id + 1];
	}
	else if( (threadIdx.x == 0) && (threadIdx.y > 0) && (threadIdx.y < blockDim.y - 1) ){
		a_shared[ sid - 1 ] = reference[id - 1];
	}
	else if( (threadIdx.x == 0) && (threadIdx.y == 0)){
		a_shared[sid - sdimx - 1] = reference[id - (nx) - 1];
		a_shared[sid - sdimx] = reference[id - (nx)];
		a_shared[sid - 1] = reference[id -1];
	}
