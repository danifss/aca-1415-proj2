...
__global__ void CUDA_non_maximum_supression_kernel(pixel_t *A, pixel_t *B, pixel_t *C, pixel_t *out, int nx, int ny)
{
	...
	float d = (float)(fmod(atan2((double)B[id],(double)A[id]) + M_PI, M_PI) / M_PI) * 8;

	if (((d <= 1 || d > 7) && C[id] > C[ee] && C[id] > C[ww]) || ((d > 1 && d <= 3) && C[id] > C[nw] && C[id] > C[se]) ||((d > 3 && d <= 5) && C[id] > C[nn] &&	 C[id] > C[ss]) || ((d > 5 && d <= 7) && C[id] > C[ne] && C[id] > C[sw])){
		out[id] = C[id];
	}
}
