...
{	
...

	{
		...
		nbs[0] = sid - blockDim.x;     // nn
		nbs[1] = sid + blockDim.x;     // ss
		nbs[2] = sid + 1;      // ww
		nbs[3] = sid - 1;      // ee
		nbs[4] = nbs[0] + 1; // nw
		nbs[5] = nbs[0] - 1; // ne
		...
		if (nms[id] >= tmin && a_shared[sid] == 0) {
			for(int k = 0; k < 8; k++){
				if (a_shared[nbs[k]] != 0) {
					a_shared[sid] = MAX_BRIGHTNESS;
					*pchanged = true;
					break;
				}
			}
			reference[id] = a_shared[sid];
				  ...
				  int * h_odata)
{
	...
	convolution(h_odata, after_Gx,(float*) Gx, nx, ny, 3); // convolution function in CUDA
	...
	convolution(h_odata, after_Gy, (float *) Gy, nx, ny, 3); // convolution function in CUDA
