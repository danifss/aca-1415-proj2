...
__global__ void CUDA_convolution_kernel(pixel_t *A, pixel_t *B, float *kirnel, int nx, int ny, int kn)
{
	...
	for (int j = -khalf; j <= khalf; j++){
		for (int i = -khalf; i <= khalf; i++) {
			...
			pixel += tex1Dfetch(A_texture,(n + j) * nx + m + i) * kirnel[c];
			c++;
		}
	}
	B[id] = (pixel_t)pixel;
...
__global__ void CUDA_non_maximum_supression_kernel(pixel_t *A, pixel_t *B, pixel_t *C, pixel_t *out, int nx, int ny)
{
	...
	// fetch texture
	pixel_t C_texture_fetched[9];
	
	C_texture_fetched[0] = tex1Dfetch(C_texture,nn);
	C_texture_fetched[1] = tex1Dfetch(C_texture,ss);
	C_texture_fetched[2] = tex1Dfetch(C_texture,ww);
	C_texture_fetched[3] = tex1Dfetch(C_texture,ee);
	C_texture_fetched[4] = tex1Dfetch(C_texture,nw);
	C_texture_fetched[5] = tex1Dfetch(C_texture,ne);
	C_texture_fetched[6] = tex1Dfetch(C_texture,sw);
	C_texture_fetched[7] = tex1Dfetch(C_texture,se);
	C_texture_fetched[8] = tex1Dfetch(C_texture,c);


	if (((dir <= 1 || dir > 7) && C_texture_fetched[8] > C_texture_fetched[3] &&
		 C_texture_fetched[8] > C_texture_fetched[2]) || // 0 deg
		((dir > 1 && dir <= 3) && C_texture_fetched[8] > C_texture_fetched[4] &&
		 C_texture_fetched[8] > C_texture_fetched[7]) || // 45 deg
		((dir > 3 && dir <= 5) && C_texture_fetched[8] > C_texture_fetched[0] &&
		 C_texture_fetched[8] > C_texture_fetched[1]) || // 90 deg
		((dir > 5 && dir <= 7) && C_texture_fetched[8] > C_texture_fetched[5] &&
		 C_texture_fetched[8] > C_texture_fetched[6]))   // 135 deg
		out[c] = C_texture_fetched[8];
	else
		out[c] = 0;
}
...
__global__ void CUDA_hysteresis_edges_kernel(const pixel_t *nms, pixel_t *reference, const int nx, const int ny, const int tmin, bool *pchanged)
{
	...
	if ((xx>0 && xx<nx-1) && (yy>0 && yy<ny-1))
	{
		...
		if (nms[id] >= tmin && tex1Dfetch(nbs_texture,id) == 0) {
			for(int k = 0; k < 8; k++){
				//if (reference[nbs[k]] != 0) {
				if (tex1Dfetch(nbs_texture,nbs[k]) != 0) {
