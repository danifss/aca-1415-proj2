...
__global__ void CUDA_non_maximum_supression_kernel(pixel_t *A, pixel_t *B, pixel_t *C, pixel_t *out, int nx, int ny)
{
	...
	int ids = threadIdx.x + threadIdx.y * blockDim.x;
	...
	// fetch array texture
	pixel_t C_texture_fetched[BLOCKSIZE];
	
	C_texture_fetched[ids] = tex1Dfetch(C_texture,ids);


	if (((dir <= 1 || dir > 7) && C_texture_fetched[c] > C_texture_fetched[ee] &&
		 C_texture_fetched[c] > C_texture_fetched[ww]) || // 0 deg
		((dir > 1 && dir <= 3) && C_texture_fetched[c] > C_texture_fetched[nw] &&
		 C_texture_fetched[c] > C_texture_fetched[se]) || // 45 deg
		((dir > 3 && dir <= 5) && C_texture_fetched[c] > C_texture_fetched[nn] &&
		 C_texture_fetched[c] > C_texture_fetched[ss]) || // 90 deg
		((dir > 5 && dir <= 7) && C_texture_fetched[c] > C_texture_fetched[ne] &&
		 C_texture_fetched[c] > C_texture_fetched[sw]))   // 135 deg
		out[c] = C_texture_fetched[c];
	else
		out[c] = 0;
}
...
void CUDA_non_maximum_supression(const pixel_t *after_Gx, const pixel_t * after_Gy, const pixel_t *G, pixel_t *nms, const int nx, const int ny)
{
	...
	// texture
//	cudaChannelFormatDesc channel = cudaCreateChannelDesc<pixel_t>();
//	cudaArray *cuArray;
//	cudaMallocArray(&cuArray, &channel, memsize);
//	cudaMemcpyToArray(cuArray, 0, 0, G, memsize, cudaMemcpyHostToDevice);
	
//	C_texture.filterMode = cudaFilterModeLinear;
//	C_texture.addressMode[0] = cudaAddressModeWrap;
//	C_texture.addressMode[1] = cudaAddressModeClamp;

//	cudaBindTextureToArray(C_texture,cuArray);
